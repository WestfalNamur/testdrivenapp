...
# run e2e tests
e2e() {
  docker-compose -f testdrivenapp/docker-compose-$1.yml up -d --build
  docker-compose -f testdrivenapp/docker-compose-$1.yml run users python manage.py recreate_db
  ./node_modules/.bin/cypress run --config baseUrl=http://127.0.0.1
  inspect $? e2e
  docker-compose -f testdrivenapp/docker-compose-$1.yml down
}
